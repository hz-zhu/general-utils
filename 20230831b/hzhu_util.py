...
class HUtil:
    ...
    def check_type(
        ...
        if self_check:
            HUtil.check_type(raise_error, bool, raise_error=True, self_check=False)
            HUtil.check_type(self_check, bool, raise_error=True, self_check=False)
            if HUtil.is_iterable(dtype):
                HUtil.check_type(dtype, tuple, raise_error=True, self_check=False)
                for item in dtype:
                    HUtil.check_type(item, type, raise_error=True, self_check=False)
            else:
                HUtil.check_type(dtype, type, raise_error=True, self_check=False)
    ...
    @staticmethod
    def check_iterable(
        x: Any, raise_error: bool = True, self_check: bool = True
    ) -> bool:
        return HUtil.check_type(
            x=x, dtype=abc.Iterable, raise_error=raise_error, self_check=self_check
        )
    ...
    def check_callable(
        ...
        return HUtil.check_type(
            x=x, dtype=abc.Callable, raise_error=raise_error, self_check=self_check
        )
    ...
    def check_iterable_type(
        ...
        for i, item in enumerate(x):
            if not HUtil.check_type(
                item,
                dtype=dtype,
                raise_error=raise_error,
                self_check=False if i else True,
            ):
                return False
        return True
    ...
    def str_contains(
        s: str,
        contain_all: Iterable[str] = "",
        contain_any: Iterable[str] = "",
        contain_none: Iterable[str] = "",
    ) -> bool:
    ...
    def ls(
        path: str = os.getcwd(),
        full_dir=False,
        sort_key=lambda x: (HUtil.extract_numbers(x), x),
    ) -> List[str]:
    ...
    def ls_file(
        ...
        return [
            rf if full_dir else r
            for r, rf in zip(result, result_full)
            if os.path.isdir(rf)
        ]
    ...
    def extract_numbers(input_string) -> List[Union[int, float]]:
        ...
        numbers = [
            float(number) if "." in number or "e" in number.lower() else int(number)
    ...
    def ls_name(
        ...
        *,
        ...
        result = list(
            filter(
                filter_func, HUtil.ls_file(path=path, full_dir=False, sort_key=sort_key)
            )
        )
        result_full = [os.path.join(path, item) for item in result]
    ...
    def disp_struct_full(x: Any) -> None:
        def recurse(x, depth):
            ...
            if is_dataclass(x):
                ...
                return (
                    "[" + ",".join((recurse(item, depth=depth + 1) for item in x)) + "]"
                )
            else:
    ...
    @staticmethod
    def cls_pre_func_decorator(pre_func: Callable, *pre_args, **pre_kwargs):
        # For class methods only (pre_func and func need to be from the same class without nested decorator)
        # Calls a new method "pre_func" with arguments *pre_args, **pre_kwargs before running the decorated function
        hutil.check_callable(pre_func)

        def func_wrapper(func: Callable):
            hutil.check_callable(pre_func)
            if func.__qualname__.split(".")[0] != pre_func.__qualname__.split(".")[0]:
                raise TypeError(
                    f"Expecting {func.__qualname__ = } and {pre_func.__qualname__ = } to be from the same class."
                )

            def wrapper(self, *args, **kwargs):
                pre_func(self, *pre_args, **pre_kwargs)
